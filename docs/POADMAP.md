...
┌─────────────────┐ ┌──────────────────┐ ┌─────────────────┐
│ 模型调度器 │────│ 负载均衡器 │────│ 模型适配器 │
│ Model Router │ │ Load Balancer │ │ Model Adapter │
└─────────────────┘ └──────────────────┘ └─────────────────┘
│ │ │
▼ ▼ ▼
┌─────────────────┐ ┌──────────────────┐ ┌─────────────────┐
│ 通义千问 │ │ GPT-4/3.5 │ │ 其他模型 │
│ Qwen Model │ │ OpenAI API │ │ Custom Models │
└─────────────────┘ └──────────────────┘ └─────────────────┘

### 身份识别系统流程

消息输入 → 身份检测器 → 策略路由器 → 专家选择器 → 响应生成
│ │ │ │ │
│ ▼ │ ▼ ▼
│ 卖家/买家 │ 卖家策略 卖家专家
│ 角色识别 │ 买家策略 买家专家
...
┌──────────────┐ ┌─────────────────┐ ┌──────────────────┐
│ 消息接收 │────│ 消息队列 │────│ 消息处理器 │
│ Message Input│ │ Message Queue │ │ Message Processor│
└──────────────┘ └─────────────────┘ └──────────────────┘
│ │
▼ ▼
┌─────────────────┐ ┌──────────────────┐
│ 消息路由 │ │ 响应生成器 │
│ Message Router │ │ Response Generator│
└─────────────────┘ └──────────────────┘
